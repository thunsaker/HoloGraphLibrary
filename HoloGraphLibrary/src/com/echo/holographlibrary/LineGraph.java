...
public class LineGraph extends View {
    ...
    public void onDraw(Canvas canvas) {
        ...
        mPaint.reset();
        float bottomPadding = 30 * getResources().getDisplayMetrics().scaledDensity, topPadding = 10;
        float sidePadding = 10 * getResources().getDisplayMetrics().scaledDensity;
        if (mUseDips) {
        ...
        for (Line line : mLines) {
            ...
            for (LinePoint p : line.getPoints()) {
                if (p.getXAxisLabel() != null) {
                    ...
                    if (count == 0) {
                        lastXPixels = sidePadding + (xPercent * usableWidth);
                        lastYPixels = getHeight() - (usableHeight * yPercent);
                    } else {
                        newXPixels = sidePadding + (xPercent * usableWidth);
                        lastXPixels = newXPixels;
                    }
                    ...
                    float textWidth = mPaint.measureText(p.getXAxisLabel());
                    int x = (int) ((lastXPixels) - (textWidth / 2));
                    int y = (int) (lastYPixels - (2 * getResources().getDisplayMetrics().scaledDensity));
                    canvas.drawText(p.getXAxisLabel(), x, y, mPaint);
                }
                count++;
            }
        }
